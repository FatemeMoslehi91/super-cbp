...
tiv_df = pd.read_csv("/home/fmoslehi/SuperCBP/HCP_YA_TIV.csv")
tiv_df.columns = [col.strip() for col in tiv_df.columns]  # Remove extra spaces

# Assumption: TIV file columns are like this: ['Subject', 'TIV']
tiv_df['Subject'] = tiv_df['Subject'].astype(str)
tiv_dict = tiv_df.set_index('Subject')['TIV'].to_dict()

def is_valid_solution(chromosome, voxel_coords, num_clusters):
    """
    Check if a solution is valid (no empty clusters, centers within bounds, and balanced cluster sizes).
    
    :param chromosome: Chromosome to check
    :param voxel_coords: Array of voxel coordinates
    :param num_clusters: Number of clusters
    :return: Boolean indicating if solution is valid
    """
    try:
        ...
        for center in centers:
            if not (x_bounds[0] <= center[0] <= x_bounds[1] and
                   y_bounds[0] <= center[1] <= y_bounds[1] and
                   z_bounds[0] <= center[2] <= z_bounds[1]):
                return False
        
        # Initialize K-means and get assignments
        ...
        # Calculate expected cluster size and allowed deviation
        total_voxels = len(voxel_coords)
        expected_size = total_voxels / num_clusters
        min_size = expected_size * 0.10  # Minimum 15% of expected size
        max_size = expected_size * 1.90  # Maximum 185% of expected size
        ...
        print(f"Error in is_valid_solution: {str(e)}")
        return False

def initialize_population(pop_size, num_clusters, brain_bounds, voxel_coords):
    """
    ...
    :param voxel_coords: Array of voxel coordinates for validation
    ...
    """
    population = []
    max_attempts = pop_size * 50  # Increased maximum attempts
    attempts = 0
    
    # Define fixed bounds for cluster centers
    """voxel_bounds = [
        [-54.0, 56.0],   # x bounds
        [-42.0, 14.0],   # y bounds
        [-48.0, 26.0]    # z bounds
    ...
    print("\nInitializing population with fixed bounds:")
    print(f"X range: [{voxel_bounds[0][0]}, {voxel_bounds[0][1]}]")
    print(f"Y range: [{voxel_bounds[1][0]}, {voxel_bounds[1][1]}]")
    print(f"Z range: [{voxel_bounds[2][0]}, {voxel_bounds[2][1]}]")
    
    # Extract min and max bounds for each dimension
    min_bound = np.repeat([bound[0] for bound in voxel_bounds], num_clusters)
    max_bound = np.repeat([bound[1] for bound in voxel_bounds], num_clusters)
    ...
    while len(population) < pop_size and attempts < max_attempts:
        ...
        # Ensure x-coordinates are positive for right hemisphere
        for i in range(num_clusters):
            x_idx = i * 3
            if chromosome[x_idx] < 0:
                chromosome[x_idx] = abs(chromosome[x_idx])
        
        # Check if solution is valid (no empty clusters and within bounds)
        if is_valid_solution(chromosome, voxel_coords, num_clusters):
            population.append(chromosome)
            print(f"Found valid solution {len(population)}/{pop_size}")
    
    if len(population) < pop_size:
        print(f"Warning: Could only generate {len(population)} valid solutions out of {pop_size} requested")
    
    return population

def get_brain_bounds(storage_name="HCP_R1_LR_VoxExtr_TianHip.hdf5", data_path="/home/fmoslehi/SuperCBP/", padding=2):
    """
    Extract brain coordinate bounds from the HDF5 storage file and add padding in all directions.
    
    :param storage_name: Name of the HDF5 filel
    ...
    if padding is not None and padding > 0:
        # For x and z dimensions, add padding to both sides
        for i in [0, 2]:  # x and z dimensions
            brain_bounds[i][0] -= padding
            brain_bounds[i][1] += padding
        
        # For y dimension, only add padding to the positive side
        brain_bounds[1][1] += padding
    
    print("Brain bounds extracted from voxel coordinates (with padding):")
...
def evaluate_fitness(chromosome, vox_data, voxel_coords, roi_data, labels, num_clusters=4, subjects=None, generation=None, chrom_idx=None):
    ...
    for i, center in enumerate(centers):
        print(f"    Cluster {i+1}: [{center[0]:.4f}, {center[1]:.4f}, {center[2]:.4f}]")
    ...
    for i, count in enumerate(cluster_sizes):
        ...
        print(f"    Cluster {i+1}: {count} voxels ({percentage:.1f}%)")
    
    # Check if any cluster is empty
    if np.any(cluster_sizes == 0):
        print("  Warning: One or more clusters are empty. Returning AUC = 0")
        return 0.0
    ...
    # Convert labels and subjects to DataFrame for use in julearn
    data_df = pd.DataFrame(X)  # X = feature matrix created in your code
    # Create feature column names in the format 'feat_0', 'feat_1', etc.
    data_df.columns = [f'feat_{i}' for i in range(X.shape[1])]
    data_df['label'] = labels  # Gender label
    data_df['Subject'] = subjects  # List of valid subjects
    data_df['TIV'] = data_df['Subject'].map(tiv_dict)  # Map TIV

    # Remove subjects for which TIV does not exist
    data_df = data_df.dropna(subset=['TIV'])

    # Get list of feature columns
    feature_cols = [col for col in data_df.columns if col.startswith('feat_')]

    # Create X_types dictionary for julearn
    X_types = {
    ...
    creator.add('confound_removal')  # Remove the effect of confound only from features
    creator.add(StandardScaler())  # Use StandardScaler from sklearn
    creator.add(LogisticRegression(penalty='l2', max_iter=1000, solver='lbfgs'))  # Use LogisticRegression from sklearn

    # Run cross-validation with both features and confounds in X
    results = run_cross_validation(
        data=data_df,  # Provide the data DataFrame
        model=creator,  # Pass the pipeline creator as the model
        X=feature_cols + ['TIV'],  # Include both features and confounds
    ...
    print("Individual fold scores:", results['test_score'].values)
    mean_auc = float(results['test_score'].mean())
    std_auc = float(results['test_score'].std())
    print(f"Mean AUC: {mean_auc:.3f} (+/- {std_auc:.3f})")

    # Return the mean of test scores
    return mean_auc
...
def genetic_algorithm(
    ...
    population = initialize_population(pop_size, num_clusters, brain_bounds, voxel_coords)
    
    if len(population) == 0:
        print("Error: Could not generate any valid initial solutions.")
        return None, 0.0
    ...
    for generation in range(num_generations):
        ...
        while len(new_population) < pop_size:
            ...
            if len(parents) >= 2:
                ...
                # Validate offspring before adding to population
                if is_valid_solution(offspring1, voxel_coords, num_clusters):
                    new_population.append(offspring1)
                if len(new_population) < pop_size and is_valid_solution(offspring2, voxel_coords, num_clusters):
                    new_population.append(offspring2)
        
        # If we don't have enough valid solutions, generate new ones
        while len(new_population) < pop_size:
            # Generate a new chromosome
            chromosome = np.random.uniform(
                low=np.repeat([bound[0] for bound in brain_bounds], num_clusters),
                high=np.repeat([bound[1] for bound in brain_bounds], num_clusters),
                size=(num_clusters * 3)
            )
...
def save_results(chromosome, fitness, num_clusters, subjects, voxel_coords, vox_data, roi_data, labels):
    ...
    with open(f"{results_dir}/summary.txt", "w") as f:
        ...
        for i, center in enumerate(centers):
            f.write(f"Cluster {i+1}: [{center[0]:.6f}, {center[1]:.6f}, {center[2]:.6f}]\n")
