...
def get_brain_bounds(storage_name="HCP_R1_LR_VoxExtr_TianHip.hdf5", data_path="/home/fmoslehi/SuperCBP", padding=20):
    """
...
def evaluate_fitness(chromosome, vox_data, voxel_coords, roi_data, labels, num_clusters=4, subjects=None, generation=None, chrom_idx=None):
    """
    Evaluate the fitness of a chromosome using K-means clustering and logistic regression.
    ...
    try:
        ...
        print(f"  Best AUC (GridSearchCV): {best_auc:.2f}")
        print(f"  Best params: {best_params}")
        auc = round(float(best_auc), 2)
        best_pipe = grid.best_estimator_
        scores = cross_val_score(best_pipe, X, labels, cv=cv, scoring='roc_auc')
        fold_scores = [round(float(score), 2) for score in scores]
        std_auc = round(float(np.std(scores)), 2)
        print(f"  Cross-validation AUC scores: {fold_scores}")
        if chrom_idx is not None:
            print(f"  Chromosome {chrom_idx} fitness: {auc:.2f}")
    except Exception as e:
