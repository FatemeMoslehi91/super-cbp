...
# Given cluster centers (mirrored for left hemisphere)
cluster_centers = np.array([
     [-31.74873532, -8.57527202, -2.88234553],
     [-22.22351747, -26.5621381, -19.30257997],
     [-15.38424828, -23.1430896, -11.00265461],
     [-17.54775271, -28.08248937, 1.49192269]
])




def filter_left_hemisphere(voxel_coords, vox_data):
    """
    Filter voxels to keep only those in the left hemisphere (x < 0).
    ...
    """
    # Find indices of voxels in left hemisphere
    left_hemi_mask = voxel_coords[:, 0] < 0
    
    # Filter voxel coordinates
    filtered_coords = voxel_coords[left_hemi_mask]
    
    # Filter voxel data columns
    filtered_data = vox_data.iloc[:, left_hemi_mask]
    
    print(f"Filtered to {len(filtered_coords)} voxels in left hemisphere")
...
def load_data():
    ...
    voxel_coords, vox_data = filter_left_hemisphere(voxel_coords, vox_data)
...
def evaluate_clusters():
    ...
    results_dir = "clustering_results_left"
    os.makedirs(results_dir, exist_ok=True)
    
    # Open results file
    results_file = os.path.join(results_dir, "evaluation_results.txt")
    with open(results_file, "w", encoding="utf-8") as f:
        f.write("Evaluation Results for Left Hemisphere Clustering\n")
        f.write("=" * 50 + "\n\n")
        ...
        f.write("Assigning voxels to clusters...\n")
        ...
        try:
            ...
            f.write(f"Average AUC: {auc:.3f}\n")
            
            # Save cluster centers
            f.write("\nCluster Centers (X coordinates multiplied by -1):\n")
