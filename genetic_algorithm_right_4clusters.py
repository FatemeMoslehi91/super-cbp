...
def initialize_population(pop_size, num_clusters, brain_bounds):
    ...
    :param brain_bounds: List of bounds for each dimension [[x_min, x_max], [y_min, y_max], [z_min, z_max]]
    :returns: List of population's chromosomes.
    """
    population = []
    
    # Extract bounds for each dimension
    x_bounds = brain_bounds[0]
    y_bounds = brain_bounds[1]
    z_bounds = brain_bounds[2]
    ...
    for _ in range(pop_size):
        ...
        chromosome = np.zeros(num_clusters * 3)
        
        # Generate random centers within brain bounds
        for i in range(num_clusters):
            # X coordinate - ensure it's positive (right hemisphere)
            chromosome[i*3] = np.random.uniform(max(0, x_bounds[0]), x_bounds[1])
            # Y coordinate
            chromosome[i*3+1] = np.random.uniform(y_bounds[0], y_bounds[1])
            # Z coordinate
            chromosome[i*3+2] = np.random.uniform(z_bounds[0], z_bounds[1])
...
def tournament_selection(population, fitness_scores, num_parents, tournsize=4):
    """
...
def mutation(chromosome, mutation_rate, mutation_scale):
    ...
    for i in range(len(mutated)):
        if random.random() < mutation_rate:
            ...
            # Ensure x-coordinates (every 3rd element starting from 0) remain positive
            if i % 3 == 0:
                mutated[i] = max(0.1, mutated[i])  # Keep x positive with a small minimum value
    
    return mutated

def decode_chromosome(chromosome, num_clusters=4):
    """
...
def calculate_cluster_time_series(vox_data, cluster_labels, n_clusters=2):
    """
    ...
    for subject_id in subjects:
        ...
        for cluster_id in range(n_clusters):
            ...
            # Skip if no voxels in this cluster
            if len(voxel_indices) == 0:
                print(f"  Warning: No voxels in cluster {cluster_id} for subject {subject_id}")
                # Add a zero signal as placeholder
                zero_signal = np.zeros(len(subject_data))
            ...
            if cluster_voxels.isna().any().any() or np.isinf(cluster_voxels.values).any():
                print(f"  Warning: Found NaN or Inf values in cluster {cluster_id} for subject {subject_id}. Filling with 0.")
                cluster_voxels = cluster_voxels.fillna(0)
                # Replace inf with 0
                cluster_voxels = cluster_voxels.replace([np.inf, -np.inf], 0)
            
            avg_signal = cluster_voxels.mean(axis=1)
            
            # Final check for NaN or Inf
            if avg_signal.isna().any() or np.isinf(avg_signal.values).any():
                print(f"  Warning: After averaging, still found NaN or Inf in cluster {cluster_id} for subject {subject_id}")
                avg_signal = avg_signal.fillna(0).replace([np.inf, -np.inf], 0)
        ...
        try:
            ...
            # Final verification - replace any remaining NaN or Inf with zeros
            if np.isnan(cluster_time_series[subject_id]).any() or np.isinf(cluster_time_series[subject_id]).any():
                print(f"  Warning: Final check - replacing NaN/Inf values for subject {subject_id}")
                cluster_time_series[subject_id] = np.nan_to_num(cluster_time_series[subject_id], nan=0.0, posinf=0.0, neginf=0.0)
        except Exception as e:
            print(f"  Error creating time series for subject {subject_id}: {str(e)}")
            # Create a valid time series filled with zeros
            cluster_time_series[subject_id] = np.zeros((len(subject_data), n_clusters))
...
def build_feature_matrix(cluster_time_series, roi_data, subjects=None):
    ...
    for s_idx, subject_id in enumerate(subject_ids):
        ...
        for c_idx in range(n_clusters):
            ...
            # Check and clean cluster signal from NaN or Inf values
            if np.isnan(c_signal).any() or np.isinf(c_signal).any():
                print(f"  Warning: Found NaN or Inf in cluster signal for subject {subject_id}. Cleaning data...")
                c_signal = np.nan_to_num(c_signal, nan=0.0, posinf=0.0, neginf=0.0)
            
            for r_idx in range(n_rois):
                r_signal = roi_ts[:, r_idx]
                
                # Check and clean ROI signal from NaN or Inf values
                if np.isnan(r_signal).any() or np.isinf(r_signal).any():
                    print(f"  Warning: Found NaN or Inf in ROI signal for subject {subject_id}, ROI {r_idx}. Cleaning data...")
                    r_signal = np.nan_to_num(r_signal, nan=0.0, posinf=0.0, neginf=0.0)
                
                # Calculate correlation
                try:
                    # Make sure both signals have valid values
                    if len(c_signal) > 0 and len(r_signal) > 0 and not np.all(c_signal == 0) and not np.all(r_signal == 0):
...
def evaluate_fitness(chromosome, vox_data, voxel_coords, roi_data, labels, num_clusters=4, subjects=None, generation=None, chrom_idx=None):
    ...
    # Calculate initial distances to centers
    ...
    # If any cluster is too small, try to redistribute points
    min_cluster_size = 5
    max_attempts = 3
    attempt = 0
    
    while np.any(cluster_sizes < min_cluster_size) and attempt < max_attempts:
        # Find small and large clusters
        small_clusters = np.where(cluster_sizes < min_cluster_size)[0]
        large_clusters = np.where(cluster_sizes > min_cluster_size * 2)[0]
        ...
        for small_cluster in small_clusters:
            ...
            center_distances = np.array([
                np.sqrt(np.sum((centers[small_cluster] - centers[large_cluster])**2))
                for large_cluster in large_clusters
            ])
            nearest_large_idx = large_clusters[np.argmin(center_distances)]
            
            # Get points from the large cluster
            large_cluster_points = np.where(cluster_labels == nearest_large_idx)[0]
            
            # Calculate distances to both clusters
            dist_to_small = distances[large_cluster_points, small_cluster]
            dist_to_large = distances[large_cluster_points, nearest_large_idx]
            
            # Find points that are closer to the small cluster
            closer_to_small = dist_to_small < dist_to_large * 1.2  # 20% threshold
            
            # Select points to move
            points_to_move = large_cluster_points[closer_to_small]
            num_points_needed = min_cluster_size - cluster_sizes[small_cluster]
            if len(points_to_move) > num_points_needed:
                # Sort by distance to small cluster and take the closest ones
                sorted_indices = np.argsort(dist_to_small[closer_to_small])
                points_to_move = points_to_move[sorted_indices[:num_points_needed]]
            
            # Move points to small cluster
            cluster_labels[points_to_move] = small_cluster
    ...
    # Check if we still have small clusters
    if np.any(cluster_sizes < min_cluster_size):
        print("  Warning: Some clusters have less than 50 voxels. Returning low fitness score.")
        return 0.0
...
def load_data():
    ...
    # Filter voxels to include only those in the right hemisphere (x > 0)
    right_hemi_mask = voxel_coords[:, 0] > 0
    right_voxel_coords = voxel_coords[right_hemi_mask]
    right_voxel_columns = [vox_data.columns[i] for i in range(len(voxel_coords)) if right_hemi_mask[i]]
    vox_data = vox_data[right_voxel_columns]
    
    print(f"Filtered to {len(right_voxel_coords)} right hemisphere voxels (x > 0)")
    ...
    return vox_data, right_voxel_coords, roi_data, np.array(labels), valid_subjects
...
def save_results(chromosome, fitness, num_clusters, subjects, voxel_coords, vox_data, roi_data, labels):
    ...
    np.save("clustering_results/right_hemi_best_chromosome.npy", chromosome)
    ...
    cluster_data.to_csv("clustering_results/right_hemi_cluster_assignments.csv", index=False)
    
    # Create and save 3D visualization
    plot_clusters_3d(voxel_coords, cluster_labels, centers)
    
    # Save a summary report
    with open("clustering_results/right_hemi_summary.txt", "w") as f:
        f.write("RIGHT HEMISPHERE ANALYSIS RESULTS\n")
        f.write("================================\n\n")
    ...
    print(f"Summary report saved to clustering_results/right_hemi_summary.txt")
    print(f"Cluster assignments saved to clustering_results/right_hemi_cluster_assignments.csv")
    print(f"Best chromosome saved to clustering_results/right_hemi_best_chromosome.npy")
...
if __name__ == "__main__":
    ...
    print("\nFinal Results (Right Hemisphere Only):")
    print(f"Best fitness (AUC): {best_fitness:.3f}")
    
    # Best chromosome is already saved in save_results function
    print("Best chromosome saved to 'clustering_results/right_hemi_best_chromosome.npy'") 